...
function setupGauge() {
  ...
  card.innerHTML = `
    ...
    <div style="position: relative; right: 0; bottom: 10%;width: 95%; margin: auto;">
      <canvas id="temperatureGaugeCanvas" style="width: 100%; height: auto;"></canvas>
      <div id="temperatureGaugeCenter"
           style="position: absolute; top: 65%; left: 50%; transform: translate(-50%, -50%);
                  font-size: 250%; font-weight: bold; color: #444;">--</div>
      <div style="position: absolute; left: 0; bottom: 10%; font-size: 150%;">0°C</div>
      <div style="position: absolute; right: 0; bottom: 10%; font-size: 150%;">100°C</div>
    </div>
  `;

  const ctx = document.getElementById("temperatureGaugeCanvas").getContext("2d");
  ...
  const label = document.getElementById("temperatureGaugeCenter");
  onValue(ref(db, "temperature"), snapshot => {
    ...
    if (!isNaN(value)) {
      ...
      gaugeChart.update();
      label.textContent = value.toFixed(2) + "°C";
    }
  });
}
